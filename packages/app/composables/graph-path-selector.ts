...
export function useGraphPathSelector<T extends { id: string }>(options: {
  ...
  const fuse = ref<ComputedRefWithControl<Fuse<T>>>() as unknown as Ref<
    ComputedRefWithControl<Fuse<T>>
  >;
...
export function useGraphPathManager<
  T extends { id: string; imports: Record<string, any>[] },
>(options: {
  ...
  const { onToggle, dataMap, list, importIdKey } = options;
  ...
  const normalizedGraph = computed(() => {
    ...
    if (!start && !end) return list.value;
    ...
    const bfs = (startId: string, getNext: (id: string) => string[], stopAt?: string) => {
      ...
      while (queue.length > 0) {
        ...
        if (stopAt) {
          if (id === stopAt) pathMap.get(id)?.forEach((nodeId) => linkedNodes.add(nodeId));
        } else {
          linkedNodes.add(id);
        }
        ...
        if (!stopAt || id !== stopAt) {
          getNext(id).forEach((nextId) => {
            if (!visited.has(`${nextId}`)) {
              queue.push(`${nextId}`);
              if (stopAt) pathMap.set(`${nextId}`, [...(pathMap.get(id) || []), `${nextId}`]);
            }
          });
        }
      }
