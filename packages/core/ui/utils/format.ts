...
export function formatDuration(
  ...
  if (ms == null) duration = ['', '-'];
  else if (ms < 1) duration = ['<1', 'ms'];
  else if (ms < 1000) duration = [ms.toFixed(0), 'ms'];
  else if (ms < 1000 * 60) duration = [(ms / 1000).toFixed(1), 's'];
  else duration = [(ms / 1000 / 60).toFixed(1), 'min'];
