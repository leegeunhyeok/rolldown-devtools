...
export class MaybeWeakMap<K, V> implements WeakMap<any, V> {
  ...
  delete(key: K): boolean {
    if (this._weakMap.has(key)) return this._weakMap.delete(key);
    return this._map.delete(key);
  }

  has(key: K): boolean {
    if (this._weakMap.has(key)) return true;
    return this._map.has(key);
  }

  set(key: K, value: V): this {
    if (this._weakMap.has(key)) this._weakMap.set(key, value);
    else this._map.set(key, value);
    return this;
  }

  get(key: K): V | undefined {
    if (this._weakMap.has(key)) return this._weakMap.get(key);
...
export class FixedTupleMap<K extends readonly any[], V> {
  ...
  constructor(public readonly length: K['length']) {
  ...
  *_traverseMap() {
    function* traverse(
      ...
      keys: any[],
      depthLeft: number,
    ): Generator<{ keys: any[]; map: Map<any, V> }> {
      if (depthLeft === 0) {
        yield { keys, map };
      } else {
...
export function makeCachedFunction<T extends (...args: any[]) => any>(
  fn: T,
): T & { cache: TupleMap<Parameters<T>, ReturnType<T>> } {
