...
const normalizedPercent = computed(() =>
  Math.abs(((props.current - props.previous) / props.previous) * 100).toFixed(
    isNotChanged.value ? 0 : 2,
  ),
);
const trendSymbol = computed(() =>
  isNotChanged.value ? '' : props.current > props.previous ? '+' : '-',
);
...
<template>
  <span text-4 mt-1 font-mono :class="comparisonColorClass">
